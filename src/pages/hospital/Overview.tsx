...
import { fetchHospitalEmergencies } from '@/hospital-dashboard/services/api';
import { AlertTriangle, Activity, Droplet, CheckCircle, FileText } from 'lucide-react';
...
export default function Overview() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setSummary({
        active: items.filter((e: any) => e.status === 'created').length,
        pendingApproval: items.filter((e: any) => e.status === 'pending_approval').length,
        bloodUnitsRequested: items.reduce((sum: number, e: any) => sum + (e.units_required || 0), 0),
        inProgress: items.filter((e: any) => e.status === 'in_progress').length,
        closed: items.filter((e: any) => e.status === 'fulfilled').length
      });
    })();
  }, []);
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          {emergencies.length === 0 ? (
            <Card>
              <CardContent className="pt-6 text-center text-muted-foreground">No active emergencies</CardContent>
            </Card>
          ) : (
            emergencies.map((em, idx) => (
              <motion.div key={em.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }}>
                <Card className="hover:shadow-md transition-shadow">
                  <CardContent className="pt-6">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <Badge variant={em.urgency_level || 'default'}>{em.urgency_level || 'STANDARD'}</Badge>
