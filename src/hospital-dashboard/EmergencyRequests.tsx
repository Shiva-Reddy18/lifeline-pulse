 ...
 * Purpose: Manage all incoming emergency blood requests
 * 
 * Contains:
 * - Emergency ID & timestamp
 * - Blood group requirement & units needed
 * - Severity level (color-coded: critical, warning, stable)
 * - Patient district location (masked identity for privacy)
 * - Admin approval status indicators
 * 
 * Actions available:
 * - View full case details
 * - Request Admin Approval (locks case for processing)
 * - Reject or Reroute emergency to another hospital
 * 
 * CRITICAL RULE: Hospital cannot proceed without admin approval
 * Once approval is requested, the case becomes locked for modifications
 */
...
import { fetchHospitalEmergencies, requestAdminApproval } from './services/api';
...
export default function EmergencyRequests() {
  ...
  const handleRequestApproval = async (em: any) => {
    const result = await requestAdminApproval({
      emergencyId: em.id,
      hospitalId: 'hosp-1',
      doctorRemarks: 'Medical requirement validated'
    });
    if (result.success) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          >
            Active ({items.filter(statusFilter.active).length})
          </Button>
          <Button
          ...
          >
            Pending Approval ({items.filter(statusFilter.pending).length})
          </Button>
          <Button
                  ...
                  <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              Patient ID: {maskPatientId(em.patient_id || 'unknown')}
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      <div className="flex items-center gap-2 text-muted-foreground">
                        ...
                        {em.location?.address || 'Location TBD'}
                      </div>
                      <div className="flex items-center gap-2 text-muted-foreground">
                        <Clock className="w-4 h-4" />
                        Raised {Math.floor((Date.now() - new Date(em.created_at).getTime()) / 60000)} min ago
                      </div>
                      <div className="flex items-center gap-2 text-muted-foreground">
                        <Droplet className="w-4 h-4" />
                        {em.blood_group} • {em.units_required} units
                      </div>
                    </div>
                    ...
                    {em.expires_at && (
                      <CountdownTimer
                        expiresAt={new Date(em.expires_at)}
                        urgency={em.urgency_level}
                        label="Expires in"
                      />
                    )}
                    ...
                    {em.status === 'created' && (
                      <div className="flex items-center gap-2 pt-2 border-t">
                        <Button
                          ...
                          onClick={() => navigate(`/hospital/requests/${em.id}`)}
                        >
                        ...
                        <Button
                          ...
                          size="sm"
                          className="flex-1 gap-2"
                          onClick={() => handleRequestApproval(em)}
                        >
                    ...
                    {em.status === 'pending_approval' && (
                      <div className="flex items-center gap-3 pt-2 border-t bg-status-warning/5 -mx-6 -mb-6 p-4 rounded-b">
                        ...
                        <span className="text-sm font-medium text-status-warning">
                          Awaiting admin decision — case is locked for modifications
                        </span>
                      </div>
                    )}
                  </div>
